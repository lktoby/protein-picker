...
これで **アプリ（web）と PostgreSQL（db）** が起動します。

**初回だけ**、別のターミナルでデータベースの準備をします（テーブル作成とサンプルデータ投入）。
...
| コマンド | 内容 |
|---------|------|
| `npm run db:setup` | マイグレーション ＋ サンプルデータ投入（初回・スキーマ変更後） |
| `npm run db:migrate` | マイグレーションのみ適用 |
| `npm run db:seed` | サンプルデータの再投入（何度実行しても重複しません） |
| `npm run db:generate` | スキーマを変更したあとにマイグレーション SQL を生成 |
