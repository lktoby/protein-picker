...
type PurchaseFormClientProps = {
  productId: string;
  shopId?: string;
  from?: string;
};

export default function PurchaseFormClient({ productId, shopId, from }: PurchaseFormClientProps) {
  const product = findProduct(productId);
  const shop = product?.onlineShops.find((s) => s.id === shopId) ?? product?.onlineShops[0];
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <Link
        href={`/prototype/products/${product.id}${
          fromRecommend && from ? `?from=${encodeURIComponent(from)}` : ""
        }`}
